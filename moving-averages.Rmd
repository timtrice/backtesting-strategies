...
```{r 2-1-per-trade-stats}
pts <- data.frame()
for(symbol in symbols) {
    pts <- bind_rows(pts, perTradeStats(Cl.SMA20.Crossover, symbol) %>% 
                         mutate(Symbol = symbol))
}
```

```{r 2-1-trade-stats-symbol}
knitr::kable(pts %>% 
                 group_by(Symbol) %>% 
                 summarise(Trades = n(), 
                           AvgCost = mean(Max.Notional.Cost), 
                           AvgPL = mean(Net.Trading.PL), 
                           MinPL = min(Net.Trading.PL), 
                           MaxPL = max(Net.Trading.PL)), 
...
```{r 2-1-annualized-risk-return-chart, fig.cap = "Annualized Risk and Return"}
...
```{r 2-1-asummary, fig.cap = "Account Summary"}
a <- getAccount(Cl.SMA20.Crossover)
